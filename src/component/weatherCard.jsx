...
function WeatherCard({address,backgroundImage}){
    ...
    const [timer, setTimer] = useState(moment(Date.now()).format("YYYY年MM月DD日 HH时mm分ss秒"));
  
    useEffect(() => {
        const timeoutId = setTimeout(() => {
          setRefresh((r) => r + 1);
          setTimer(moment().tz(data.timezone).format("YYYY年MM月DD日 HH时mm分ss秒"));
        }, 1000);
    ...
    return(
        ...
        >
          <p className="px-2 py-1 text-sm tracki text-white title dark:text-gray-100 uppercase dark:bg-gray-800 bg-opacity-75 rounded shadow-lg">
            {address}
          </p>
        </div>
        <div className="flex justify-between p-4">
          ...
          <div className="text-sm leadi">
            <div className="flex items-center">{pressure} hPa</div>
          </div>
        </div>
